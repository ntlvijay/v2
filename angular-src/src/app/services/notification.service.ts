...
export class NotificationService {
  ...
  getUnreadNotifications(userData) {
    ...
    return this.http.post('http://' + this.serverAddress + '/notifications/getUnreadNotifications', userData, {headers: headers}).pipe(
      map(res => res.json()));
  }
  ...
  markAsRead(data) {
    let headers = new Headers;
    headers.append('Content-Type', 'application/json');
    return this.http.post('http://' + this.serverAddress + '/notifications/markAsRead', data, {headers: headers}).pipe(
      map(res => res.json()));
  }
