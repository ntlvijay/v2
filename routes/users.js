      ...
      User.addUser(newUser, (err, user) => {
        if(err){
          ...
          const newNotification = new Notification({
            ...
            message: `Hi ${ req.body.name }, Welcome to Techforum. Take some time to update your profile`,
          });
...
router.post('/forgotPassword/username', (req, res, next) => {
  ...
  User.forgotPassword(username, (err, user) => {
    ...
    if (user) {
      res.json({
        success: true,
        // securityQuestion: user.securityQuestion,//nithil plz add email verific here
      });
    } else {
      res.json({
        success: false,
